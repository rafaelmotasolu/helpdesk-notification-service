package com.solutis.projeto.helpdesk_notification_service.security;
...
class SecurityUtilsTest {
    ...
    @Test
    void shouldReturnRoleWithoutPrefix() {
        var auth = new UsernamePasswordAuthenticationToken("1", "pass", List.of(new SimpleGrantedAuthority("ROLE_TECHNICIAN")));
        SecurityContextHolder.getContext().setAuthentication(auth);

        assertEquals("TECHNICIAN", SecurityUtils.getCurrentRole());
        assertTrue(SecurityUtils.isTechnician());
    ...
    @Test
    void shouldReturnNullAndFalseWhenNoAuthentication() {
